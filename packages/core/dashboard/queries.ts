...
export const dashboardKeys = {
  ...
  usageBreakdown: (
    ...
    [...dashboardKeys.all(wsId), "usage-breakdown", days, projectId, tz] as const,
  delivery: (
    wsId: string,
    days: number,
    projectId: string | null,
    tz: string,
  ) => [...dashboardKeys.all(wsId), "delivery", days, projectId, tz] as const,
};
...
export function dashboardUsageBreakdownOptions(
  wsId: string,
  days: number,
  projectId: string | null,
  tz: string,
) {
  const queryKey = dashboardKeys.usageBreakdown(wsId, days, projectId, tz);
  return queryOptions({
    queryKey,
    queryFn: () =>
      api.getDashboardUsageBreakdown({
        days,
        project_id: projectId ?? undefined,
        tz,
      }),
...
// Delivery reads live tables (task queue + activity log), not the rollups, so
// it refreshes on the same cadence purely to stay in step with the page.
export function dashboardDeliveryOptions(
  wsId: string,
  days: number,
  projectId: string | null,
  tz: string,
) {
  const queryKey = dashboardKeys.delivery(wsId, days, projectId, tz);
  return queryOptions({
    queryKey,
    queryFn: () =>
      api.getDashboardDelivery({
        days,
        project_id: projectId ?? undefined,
        tz,
      }),

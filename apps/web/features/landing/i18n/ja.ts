  ...
  return {
    ...
    about: {
      ...
      paragraphs: [
        ...
        "ソースコードは公開されており、無料でセルフホストできます。データはあなたのインフラに残ります。すべてのコードを確認し、API を拡張し、好きな LLM プロバイダーを持ち込み、コミュニティに貢献できます。",
      ],
      cta: "GitHub で見る",
      team: {
    ...
    privacy: {
      ...
      intro: [
        ...
        "このポリシーは、お客様がご自身でホストする Multica には適用されません。セルフホスト環境のデータは運用者が管理し、どの AI プロバイダー、連携サービス、分析ツールを使うかは運用者の設定によります。セルフホストのサーバーが当社に送信するのは、1日1回の利用状況スナップショットのみです。内容は、同じサーバーのスナップショットを関連付けるためのランダムな環境 ID、サーバーのバージョン、ワークスペース・メンバー・エージェント・接続中のデーモンのおおよその数、その日に開始・完了・失敗・キャンセルされたタスクの数です。氏名、メールアドレス、コンテンツは含まれません。DO_NOT_TRACK=1 を設定すると、このスナップショットの送信を停止できます。",
      ...
      sections: [
        {
          ...
          bullets: [
            ...
            "お客様が作成するコンテンツ: ワークスペース、Issue、コメント、チャットメッセージ、添付ファイル、エージェントへの指示など、お客様またはお客様のエージェントが Multica Cloud に入力するすべてのもの。",
        ...
        },
        {
          heading: "AI 機能",
          paragraphs: [
            "コーディングエージェントは、お客様自身のマシンやお客様が接続したランタイム上で、お客様が設定したコーディングツールとアカウントを使って動作します。エージェントがローカルで動いていても、モデルの推論までローカルで行われるわけではありません。これらのツールは、プロンプト、コード、ファイル、ツールの実行結果をそれぞれのモデルプロバイダーに送信し、その扱いはお客様が使うツールとアカウントの規約に従います。Multica はエージェントのタスクを調整します。",

...
func (q *Queries) ChildIssueProgress(ctx context.Context, arg ChildIssueProgressParams) ([]ChildIssueProgressRow, error) {
	rows, err := q.db.Query(ctx, childIssueProgress, arg.WorkflowEnabled, arg.TerminalStatusKeys, arg.WorkspaceID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
WITH wakeup_source AS MATERIALIZED (SELECT set_config('multica.source_task_id', COALESCE($4::uuid::text, ''), true)), candidate AS MATERIALIZED (
    ...
    SELECT
        ...
        COALESCE($5::text, i.title) AS next_title,
        COALESCE($6::text, i.description) AS next_description,
        COALESCE($7::text, i.status) AS next_status,
        COALESCE($8::text, i.priority) AS next_priority,
        $9::text AS next_assignee_type,
        $10::uuid AS next_assignee_id,
        CASE
        ...
        CASE
            WHEN $17::uuid IS NOT NULL
                THEN $17::uuid
            WHEN i.status = 'cancelled' AND COALESCE($7::text, i.status) = 'cancelled'
...
type UpdateIssueParams struct {
	...
	ExpectedTransitionID pgtype.UUID   `json:"expected_transition_id"`

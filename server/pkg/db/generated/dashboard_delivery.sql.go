...
const listDashboardDeliveryIssues = `-- name: ListDashboardDeliveryIssues :many
...
cohort AS (
    ...
    WHERE NOT EXISTS (
        ...
        WHERE prior.issue_id = wt.issue_id
          AND prior.created_at < $3::timestamptz
    )
),
transitions AS (
    SELECT
...
type ListDashboardDeliveryIssuesParams struct {
	WorkspaceID pgtype.UUID        `json:"workspace_id"`
	ProjectID   pgtype.UUID        `json:"project_id"`
	Since       pgtype.Timestamptz `json:"since"`
}

type ListDashboardDeliveryIssuesRow struct {
	...
	FirstTaskAt    pgtype.Timestamptz `json:"first_task_at"`
	DeliveredAt    pgtype.Timestamptz `json:"delivered_at"`
	LastDoneAt     pgtype.Timestamptz `json:"last_done_at"`
	BounceCount    int32              `json:"bounce_count"`
	LastBounceAt   pgtype.Timestamptz `json:"last_bounce_at"`
...
func (q *Queries) ListDashboardDeliveryIssues(ctx context.Context, arg ListDashboardDeliveryIssuesParams) ([]ListDashboardDeliveryIssuesRow, error) {
	rows, err := q.db.Query(ctx, listDashboardDeliveryIssues, arg.WorkspaceID, arg.ProjectID, arg.Since)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListDashboardDeliveryIssuesRow{}
	for rows.Next() {
		var i ListDashboardDeliveryIssuesRow
		if err := rows.Scan(
			&i.IssueID,
			&i.Number,
			&i.Title,
...
const listDashboardUsageBreakdown = `-- name: ListDashboardUsageBreakdown :many
SELECT
    agent_id,
    runtime_id,
    project_id,
...
type ListDashboardUsageBreakdownParams struct {
	WorkspaceID pgtype.UUID        `json:"workspace_id"`
	Since       pgtype.Timestamptz `json:"since"`
	ProjectID   pgtype.UUID        `json:"project_id"`
}

type ListDashboardUsageBreakdownRow struct {
	AgentID                  pgtype.UUID `json:"agent_id"`
...
func (q *Queries) ListDashboardUsageBreakdown(ctx context.Context, arg ListDashboardUsageBreakdownParams) ([]ListDashboardUsageBreakdownRow, error) {
	rows, err := q.db.Query(ctx, listDashboardUsageBreakdown, arg.WorkspaceID, arg.Since, arg.ProjectID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListDashboardUsageBreakdownRow{}
	for rows.Next() {
		var i ListDashboardUsageBreakdownRow
		if err := rows.Scan(
			&i.AgentID,
			&i.RuntimeID,
			&i.ProjectID,

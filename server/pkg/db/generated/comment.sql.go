...
SELECT id, issue_id, author_type, author_id, content, type, created_at, updated_at, parent_id, workspace_id, resolved_at, resolved_by_type, resolved_by_id, source_task_id, quick_action_id, via_plugin_id, revision, recovery_settled_at, deleted_at, client_request_id, suppressed_agent_ids FROM comment
WHERE id = $1
`
...
const getCommentByClientRequest = `-- name: GetCommentByClientRequest :one
SELECT id, issue_id, author_type, author_id, content, type, created_at, updated_at, parent_id, workspace_id, resolved_at, resolved_by_type, resolved_by_id, source_task_id, quick_action_id, via_plugin_id, revision, recovery_settled_at, deleted_at, client_request_id, suppressed_agent_ids FROM comment
WHERE issue_id = $1
  AND author_id = $2
...
type GetCommentByClientRequestParams struct {
	IssueID         pgtype.UUID `json:"issue_id"`
	AuthorID        pgtype.UUID `json:"author_id"`
	ClientRequestID pgtype.UUID `json:"client_request_id"`
}
...
func (q *Queries) GetCommentByClientRequest(ctx context.Context, arg GetCommentByClientRequestParams) (Comment, error) {
	row := q.db.QueryRow(ctx, getCommentByClientRequest, arg.IssueID, arg.AuthorID, arg.ClientRequestID)
	var i Comment
	err := row.Scan(
		&i.ID,
		&i.IssueID,
...
WITH RECURSIVE ancestor_path AS (
  SELECT c.id, c.issue_id, c.author_type, c.author_id, c.content, c.type, c.created_at, c.updated_at, c.parent_id, c.workspace_id, c.resolved_at, c.resolved_by_type, c.resolved_by_id, c.source_task_id, c.quick_action_id, c.via_plugin_id, c.revision, c.recovery_settled_at, c.deleted_at, c.client_request_id, c.suppressed_agent_ids, ARRAY[c.id]::uuid[] AS visited_ids, 1::integer AS depth, false AS cycle
  FROM comment c
  WHERE c.id = $1
  ...
  SELECT parent.id, parent.issue_id, parent.author_type, parent.author_id, parent.content, parent.type, parent.created_at, parent.updated_at, parent.parent_id, parent.workspace_id, parent.resolved_at, parent.resolved_by_type, parent.resolved_by_id, parent.source_task_id, parent.quick_action_id, parent.via_plugin_id, parent.revision, parent.recovery_settled_at, parent.deleted_at, parent.client_request_id, parent.suppressed_agent_ids,
...
SELECT id, issue_id, author_type, author_id, content, type, created_at, updated_at, parent_id, workspace_id, resolved_at, resolved_by_type, resolved_by_id, source_task_id, quick_action_id, via_plugin_id, revision, recovery_settled_at, deleted_at, client_request_id, suppressed_agent_ids
FROM thread_history
...
SELECT id, issue_id, author_type, author_id, content, type, created_at, updated_at, parent_id, workspace_id, resolved_at, resolved_by_type, resolved_by_id, source_task_id, quick_action_id, via_plugin_id, revision, recovery_settled_at, deleted_at, client_request_id, suppressed_agent_ids FROM (
    SELECT id, issue_id, author_type, author_id, content, type, created_at, updated_at, parent_id, workspace_id, resolved_at, resolved_by_type, resolved_by_id, source_task_id, quick_action_id, via_plugin_id, revision, recovery_settled_at, deleted_at, client_request_id, suppressed_agent_ids FROM comment
    WHERE issue_id = $1 AND workspace_id = $2
...
const setCommentSuppressedAgents = `-- name: SetCommentSuppressedAgents :exec
UPDATE comment SET suppressed_agent_ids = $1::uuid[]
WHERE id = $2
`

type SetCommentSuppressedAgentsParams struct {
	SuppressedAgentIds []pgtype.UUID `json:"suppressed_agent_ids"`
	ID                 pgtype.UUID   `json:"id"`
}

// An edit re-decides which agents the comment must not start.
func (q *Queries) SetCommentSuppressedAgents(ctx context.Context, arg SetCommentSuppressedAgentsParams) error {
	_, err := q.db.Exec(ctx, setCommentSuppressedAgents, arg.SuppressedAgentIds, arg.ID)
	return err
}

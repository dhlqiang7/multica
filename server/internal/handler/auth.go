...
type SendCodeRequest struct {
	Email string `json:"email"`
	// selfhost 密码模式（MULTICA_AUTH_MODE=password）下携带固定密码，
	// 其余模式忽略。见 auth_selfhost.go。
...
func (h *Handler) SendCode(w http.ResponseWriter, r *http.Request) {
	...
	// selfhost 直登分派（passwordless/password 模式）：处理完毕即返回，
	// code 模式继续原生验证码流程。见 auth_selfhost.go。
	if h.maybeDirectLogin(w, r, email, req.Password) {
		return
	}

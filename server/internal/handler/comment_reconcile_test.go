...
func TestCompleteTask_ReconcilesAgentAuthoredMentionToCompletedAgent(t *testing.T) {
	...
	testHandler.triggerTasksForComment(ctx, issue, mentionComment, nil, "agent", agentA, "", nil, commentSteer{})
...
func TestConsecutiveCommentsDifferentOriginatorsFullEnqueuePath(t *testing.T) {
	...
	testHandler.triggerTasksForComment(ctx, issue, cA, nil, "member", testUserID, testUserID, nil, commentSteer{})
	if n := pendingTaskCountForAgentIssue(t, issueID, agentID); n != 1 {
	...
	cB.ParentID = cA.ID
	testHandler.triggerTasksForComment(ctx, issue, cB, nil, "member", userB, userB, nil, commentSteer{})

...
const issueWorkflowMigrationTestSchema = "issue_workflow_migration_test"

func TestIssueWorkflowMigrationsBackfillIdempotentlyAndRollBack(t *testing.T) {
	...
	cleanup := func() {
		_, _ = pool.Exec(context.Background(), "DROP SCHEMA IF EXISTS "+issueWorkflowMigrationTestSchema+" CASCADE")
	}
	cleanup()
	t.Cleanup(cleanup)
	if _, err := conn.Exec(ctx, "CREATE SCHEMA "+issueWorkflowMigrationTestSchema); err != nil {
		t.Fatalf("create isolated migration schema: %v", err)
	}
	if _, err := conn.Exec(ctx, `SELECT set_config('search_path', $1, false)`, issueWorkflowMigrationTestSchema); err != nil {
	...
	assertWorkflowMigrationCount(t, ctx, conn, "issue_transition", 1)
	var phase, legacyStatus, cause string
	var issueBound, projectInherits bool
	if err := conn.QueryRow(ctx, `
		SELECT s.phase, s.legacy_status_key, t.cause,
		       i.workflow_id IS NOT NULL AND i.workflow_status_id = s.id AND i.last_transition_id = t.id,
		       p.default_issue_workflow_id IS NULL
		FROM issue i
		JOIN issue_workflow_status s ON s.id = i.workflow_status_id
	...
	`, issueID, projectID).Scan(&phase, &legacyStatus, &cause, &issueBound, &projectInherits); err != nil {
		t.Fatalf("read workflow backfill: %v", err)
	}
	...
	if err := conn.QueryRow(ctx, `
		SELECT status.spec_key, workflow.initial_status_id
		FROM issue_workflow AS workflow
		JOIN issue_workflow_status AS status ON status.id = workflow.initial_status_id
		LIMIT 1
	`).Scan(&specKey, &initialStatusID); err != nil {
		t.Fatalf("read workflow spec fields: %v", err)
	}
	if specKey != "todo" || !initialStatusID.Valid {
		t.Fatalf("workflow spec fields = key %q initial %v, want todo and a valid id", specKey, initialStatusID)
	}
	...
	if err := conn.QueryRow(ctx, `SELECT to_regclass($1) IS NOT NULL`, issueWorkflowMigrationTestSchema+".issue_workflow").Scan(&workflowExists); err != nil {
		t.Fatalf("inspect rollback: %v", err)
	}
	if workflowExists {
		t.Fatal("issue_workflow still exists after down migrations")
	}
}

func assertWorkflowMigrationCount(t *testing.T, ctx context.Context, conn *pgxpool.Conn, table string, want int) {
	t.Helper()
	var got int
